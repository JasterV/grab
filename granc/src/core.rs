...
use crate::core::client::ClientConnectError;
...
impl Granc<Channel> {
    ...
    pub async fn connect(addr: &str) -> Result<Self, ClientConnectError> {
        ...
        Ok(Self {
            service: channel,
            service_address: Some(addr.to_string()),
        })
    }
}
    ...
    <S::ResponseBody as HttpBody>::Error: Into<BoxError> + Send,
{
    ...
    pub async fn call(&self, input: CallInput) -> Result<CallOutput, GrpcCallError> {
...
) -> Result<CallOutput, GrpcCallError> {
    let result = client.unary(method, body, headers).await?;
    Ok(CallOutput::Unary(result))
}
...
async fn handle_server_stream(
    ...
    match client.server_streaming(method, body, headers).await? {
        Ok(stream) => Ok(CallOutput::Streaming(Ok(stream.collect().await))),
        Err(status) => Ok(CallOutput::Streaming(Err(status))),
    }
}
